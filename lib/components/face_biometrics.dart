...
class FaceBiometricsWidget extends StatefulWidget {
  static const String routeName = 'attendance_record_page';

  FaceBiometricsWidget({
    Key? key,
  }) : super(key: key);

  @override
  _FaceBiometricsWidget createState() => _FaceBiometricsWidget();
}

class _FaceBiometricsWidget extends State<FaceBiometricsWidget> {
  ...
  performFaceRecognition(
    ...
    for (Face face in faces) {
      ...
      if (recognition.distance > 1) {
        ...
        takeAttendance(Provider.of<SessionProvider>(context).sessionCode, '');
      }
  ...
  Future<void> takeAttendance(String sessionCode, String lecturerName) async {
    try {
      ...
      if (sessionSnapshot.docs.isEmpty) {
        ...
        setState(() {
          attendanceMessage = 'Session not found';
        });
      } else {
        // Session found
        ...
        if (distance <= sessionRadius) {
          ...
          if (existingAttendance.docs.isNotEmpty) {
            ...
            setState(() {
              attendanceMessage = 'Attendance recorded successfully';
            });
            _showDialog(
      ...
      setState(() {
        attendanceMessage = 'Error taking attendance';
      });
